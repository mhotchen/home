...
resource "aws_route53_record" "rtw-mhn-me-cert-validation" {
  for_each = {
    for dvo in aws_acm_certificate.rtw-mhn-me-cert.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "rtw-mhn-me-cert" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.rtw-mhn-me-cert-validation : record.fqdn]
  provider                = aws.us-east-1
